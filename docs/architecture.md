...
flowchart LR
    ...
    ts["Tailscale<br/>pai.tail…ts.net"] --> next
...
sequenceDiagram
    ...
    S->>S: SQLite
    loop every 2s
        B->>S: GET /api/data?profile=…
        S->>B: payload
    end
    Note over B: body differs from last poll → re-render
```
...
Snapshots share that card with the database, so they cover mistakes and
corruption but not the card dying. Settings → Your data writes the whole
profile out as a single JSON file and reads one back, which is the part that
